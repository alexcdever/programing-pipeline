...
一个与具体模型、产品和 agent runtime 无关的多代理编程工作流技能。

## 核心角色

- **主代理**：读取项目状态，设计目标与任务拆分，编写并冻结任务单，调度子代理，最终验收、合并和更新项目状态。
- **执行子代理**：在隔离 worktree 中实现冻结任务单，运行测试并写执行证据。
- **审查子代理**：使用独立上下文复验实现、验收矩阵、边界和证据，不修改产品代码。
- **监督器**：只负责长任务的存活、冻结检测、完成通知和恢复，不代替验收。
...
把本目录作为一个可加载的 agent skill，或将 `SKILL.md` 与其引用的 `templates/`、`references/`、`scripts/` 一起复制到目标 agent 的技能目录。具体的子代理调用命令、模型选择、后台监督和通知由目标运行时提供，不写入本技能。
